...
import commands, datetime, time, sys
...
def getBoundary(jobNames):
        ...
        for jobName in jobNames:
                for job in jobName.split(","):
                        ...
                        boundaryQuery = 'ssh {0} "/opt/nsn/ngdb/pgsql/bin/psql sai sairepo -c \\"select maxvalue from boundary where jobid=\'{1}\'\\"" | egrep -v "rows|row|--|maxvalue"'.format(cemod_postgres_sdk_fip_active_host,job)
                        currentBoundaryValue = commands.getoutput(boundaryQuery).split("\n")[0].strip()
