...
def regenerate_macd_with_latest_ltp(df):
    ...
    df['Signal_curr'] = calculate_ema(df['Macd'], 0.2, df['Signal'])
...
def main(sc):
    ...
    df_merged_result_selected_fields = df_merged_result_with_macd[['LTT', 'Trading_symbol', 'LTP', 'cumVol_X_Ltp', 'total_cumVolumeLtp', 'total_traded_vol', 'vwap']]
    print(df_merged_result_selected_fields)
    db_util_obj = dfDbUtil(st_constants.MARIADB_URL)
    db_util_obj.insert_db_df(df_merged_result_with_macd, 'st_analysis_vwap_macd', 'testrijdb', 'append', False)
    
    get_macd_for_df(df_merged_result_with_macd)
    
    # Schedule code to run every 3 mins
    s.enter(180, 1, main, (sc,))
