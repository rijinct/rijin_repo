...
import ST_ANALYZER_NSEPY.st_common_constants as constants
...
def get_macd_for_df(df_merged_result_with_macd):   
    numeric_col_list = ['data_0_closePrice', 'vwap']
    ...
    for idx in df_merged_result_with_macd.index:
        ltp =df_merged_result_with_macd['data_0_closePrice'][idx]
...
def check_macdCrossover(symbol,ltp,vwap,ltt):
        st_analysis_10_query = "SELECT data_0_closePrice,lastUpdateTime,STR_TO_DATE(lastUpdateTime, '%d-%M-%Y %H:%i:%s') FROM st_nse_data_5 WHERE Trading_symbol LIKE '{s}' and STR_TO_DATE(lastUpdateTime, '%d-%M-%Y') > '2020-06-01' ORDER BY STR_TO_DATE(lastUpdateTime, '%d-%M-%Y %H:%i:%s') ASC".format(s=symbol)
        print(st_analysis_10_query)
        df_st_analysis_10 = get_final_df_from_sql(st_analysis_10_query)
        macd, macdsignal, macdhist = MACD(df_st_analysis_10['data_0_closePrice'], fastperiod=12, slowperiod=26, signalperiod=9)
        macdLasVal= macd[-1]
...
def get_vwap_for_df(todays_date, nse_df):
    ...
    vwap_sql_query = constants.vwap_sql_query_5.format(todays_date='2020-07-03')
...
def get_appended_df(symbol_list):
    nse_df =  pd.DataFrame()
    for sym in symbol_list:
        nse_df_symb = nse.get_minData_df(sym)
    ...
    nse_df[['data_0_totalTradedVolume','data_0_closePrice']] = nse_df[['data_0_totalTradedVolume','data_0_lastPrice']].replace(',','').apply(pd.to_numeric)
    nse_df.rename(columns = {'data_0_symbol':'Trading_symbol'}, inplace = True)
    nse_df['cumVol_X_Ltp'] = nse_df['data_0_closePrice'] * nse_df['data_0_totalTradedVolume']
    return nse_df    
...
def main(sc):
    ...
    if (currentTime_minLastElm==7 or currentTime_minLastElm==2):
            nse_df = get_appended_df(constants.nifty50)
            print(nse_df.columns)
            exit(0)
            print('Inserting to DB')
            DF_OUTPUT_WRITE_OPERATIONS.various_write_operations.insert_db_df(nse_df, engine, 'st_nse_data_5', 'testrijdb', 'append', False)
            #get_vwap_for_df(todays_date, nse_df) 
            time.sleep(180)
    else:
            pass
